...
def send_information_about_start_habit():
    ...
    habits = Habit.objects.filter(Q(reminder_10_sent__lte=now) | Q(next_reminder__lte=now)).select_related("user")
        ...
        if habit.user.tg_chat_id:
            ...
            if habit.reminder_10_sent <= now < habit.next_reminder:
                send_telegram_message(
                    chat_id=habit.user.tg_chat_id, message=f"Напоминаем! Через 10 минут: {habit.action}."
                )
